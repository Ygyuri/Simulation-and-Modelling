...
def simulate_trajectory(mass, muzzle_velocity, launch_angle):
    ...
    return sol.y[3], sol.y[4]

# Function to check if an object is injured (reaches a specific height threshold and crosses it)
def is_injured(y_values, target_height):
    # Check if the bullet trajectory crosses the target_height
    for i in range(len(y_values) - 1):
        if (y_values[i] - target_height) * (y_values[i + 1] - target_height) <= 0:
            return True

    return False

# Animation function for updating the 2D plot with interactive elements
def animate_trajectory(i, launch_angle, target_height):
    plt.clf()
    plt.xlim(0, horizontal_distance)
    plt.ylim(0, horizontal_distance)
    plt.xlabel("Horizontal Distance (m)")
    plt.ylabel("Vertical Distance (m)")
    plt.title(f"{handguns[i]['name']} Bullet Trajectory")
    ...
    injured = is_injured(y, target_height)

    plt.plot(x, y, label="Trajectory", linestyle='-', marker='o', markersize=3)
    plt.scatter(horizontal_distance, target_height, color='red', marker='o', label="Target")

    if injured:
        plt.text(0.5, 0.5, "INJURED!", color='red', fontsize=12, transform=plt.gca().transAxes)

    plt.legend()

    # Calculate and display maximum height
    max_height, _ = calculate_max_height_distance(initial_velocity, launch_angle)
    if not np.isnan(max_height):
        plt.text(0.05, 0.9, f"Max Height: {max_height:.2f} meters", fontsize=12, transform=plt.gca().transAxes)
    else:
        plt.text(0.05, 0.9, "Max Height: N/A", fontsize=12, transform=plt.gca().transAxes)

    plt.grid()

    # Save the figure as a PNG image
    plt.savefig(f"{handguns[i]['name']}_trajectory.png")
...
# Create the plot for each handgun and save the figures
for i in range(len(handguns)):
