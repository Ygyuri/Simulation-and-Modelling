...
def simulate_trajectory(mass, muzzle_velocity):
    launch_velocity_x = 0  # Vertical launch, no horizontal component
    launch_velocity_y = 0  # Vertical launch, no horizontal component

    initial_state = [launch_velocity_x, launch_velocity_y, muzzle_velocity, 0, 0, 0]
...
def calculate_max_height_distance(initial_velocity):
...
def animate_trajectory(i, target_height):
    ax.cla()
    ...
    x, y, z = simulate_trajectory(handguns[i]['mass'], initial_velocity)
    ...
    max_height, max_distance = calculate_max_height_distance(initial_velocity)
...
def update(val):
    ...
    animate_trajectory(i, target_height)

height_slider.on_changed(update)

# Create the animation for each handgun
for i in range(len(handguns)):
    animate_trajectory(i, 50)  # Starting with a target height of 50 meters
